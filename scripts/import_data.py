...
# --- 1b. Авто-тегирование category/material/color по ключевым словам в названии ---
CATEGORY_RULES = [
    ("брелок", "Брелок"),
...
for art, (img_bytes, ext) in matched.items():
    ...
    products.append({
        ...
        "video": "",
        "visible": True,
        "category": match_first(row["name"], CATEGORY_RULES) or "Прочее",
        "material": match_first(row["name"], MATERIAL_RULES),
        "color": match_first(row["name"], COLOR_RULES),
    })
    saved += 1
