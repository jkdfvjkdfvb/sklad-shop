...
const categoryProducts = window.CATEGORY_PRODUCTS || [];
...
// ======== ADD TO CART (from category grid) ========
function addToCart(article) {
  const p = categoryProducts.find(p => p.article === article);
  if (!p) return;
...
// ======== VIDEO MODAL (unused on category page, kept for shared drawer markup) ========
const modal = document.getElementById('video-modal');
document.getElementById('modal-close').addEventListener('click', () => modal.classList.remove('open'));
modal.addEventListener('click', e => { if (e.target === modal) modal.classList.remove('open'); });
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeCart(); modal.classList.remove('open'); } });
